...
resource "aws_iam_instance_profile" "kungfu_profile" {
  name = "tf-${var.instance_name}-instance-profile_v2"
#  iam_instance_profile = aws_iam_instance_profile.kungfu_instance_profile.name
...
resource "aws_instance" "kungfu_ec2" {
  ...
  http_tokens                 = "required" # ← force l’usage d’IMDSv2
...
ingress {
  ...
  to_port     = 80
  protocol    = "tcp"
  cidr_blocks = ["0.0.0.0/0"] # ou restreint à ton IP publique si besoin
}
